...
if __name__=="__main__":
    ...
    with open('cmc9_data.pickle','rb') as f:
        data = pickle.load(f)
        left_bound = CheckDictionary(data,'left_bound')
        right_bound = CheckDictionary(data,'right_bound')
        periodic_faces = [] # CheckDictionary(data,'y_periodic')
    print(f'left bound count {len(left_bound)}')
    print(f'right bound count {len(right_bound)}')
    blocks_to_extract = [o['block_index'] for o in left_bound]
    blocks_to_extract.extend([o['block_index'] for o in right_bound])
    ...
    for i in range(len(blocks_to_extract)):
    ...
    for bindex,b in enumerate(blocks_to_extract): # Block indicies 
        ...
        for surface_indx, o in enumerate(left_bound):
            # Add Plots for Outer Faces
            if o['block_index'] == b:
                voi = [o['IMIN'], o['IMAX'], o['JMIN'], o['JMAX'],o['KMIN'], o['KMAX']]
                CreateSubset(block_source, voi, name='left_bound '+str(surface_indx),opacity=1,rgb_face_matches=rgb_outer_faces[bindex])
        
        for surface_indx, o in enumerate(right_bound):
            # Add Plots for Outer Faces
            if o['block_index'] == b:
                voi = [o['IMIN'], o['IMAX'], o['JMIN'], o['JMAX'],o['KMIN'], o['KMAX']]
                CreateSubset(block_source, voi, name='right_bound '+str(surface_indx),opacity=1,rgb_face_matches=rgb_outer_faces[bindex])
