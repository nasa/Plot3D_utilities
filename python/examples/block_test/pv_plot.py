...
if __name__=="__main__":
    ...
    '''
    rgb_outer_faces_x = list()
    for i in range(len(x_periodic_faces)):
        rgb_outer_faces_x.append([random.randint(0,255)/255, random.randint(0,255)/255, random.randint(0,255)/255])

    rgb_outer_faces_y = list()
    for i in range(len(y_periodic_faces)):
        rgb_outer_faces_y.append([random.randint(0,255)/255, random.randint(0,255)/255, random.randint(0,255)/255])

    rgb_outer_faces_z = list()
    for i in range(len(z_periodic_faces)):
        rgb_outer_faces_z.append([random.randint(0,255)/255, random.randint(0,255)/255, random.randint(0,255)/255])
    ...
    for b in blocks_to_extract: # Block indicies 
        ...
        for periodic_indx, p in enumerate(x_periodic_faces):
            ...
            if p['block1']['block_index'] == b and p['block2']['block_index'] == b: # Periodicity within the block 
                ...
                CreateSubset(block_source, voi, name='x-periodic '+str(periodic_indx),rgb_face_matches=rgb_outer_faces_x[periodic_indx])
        ...
        for periodic_indx, p in enumerate(y_periodic_faces):
            ...
            if p['block1']['block_index'] == b and p['block2']['block_index'] == b: # Periodicity within the block 
                ...
                p['block1']['KMIN'], p['block1']['KMAX'] = check_and_swap(p['block1']['KMIN'], p['block1']['KMAX'])
                voi = [p['block1']['IMIN'], p['block1']['IMAX'], p['block1']['JMIN'], p['block1']['JMAX'],p['block1']['KMIN'], p['block1']['KMAX']]
                CreateSubset(block_source, voi, name='y-periodic '+str(periodic_indx),rgb_face_matches=rgb_outer_faces_y[periodic_indx])
        ...
        for periodic_indx, p in enumerate(z_periodic_faces):
            ...
            if p['block1']['block_index'] == b and p['block2']['block_index'] == b: # Periodicity within the block 
                ...
                p['block1']['KMIN'], p['block1']['KMAX'] = check_and_swap(p['block1']['KMIN'], p['block1']['KMAX'])
                voi = [p['block1']['IMIN'], p['block1']['IMAX'], p['block1']['JMIN'], p['block1']['JMAX'],p['block1']['KMIN'], p['block1']['KMAX']]
                CreateSubset(block_source, voi, name='z-periodic '+str(periodic_indx),rgb_face_matches=rgb_outer_faces_z[periodic_indx])
